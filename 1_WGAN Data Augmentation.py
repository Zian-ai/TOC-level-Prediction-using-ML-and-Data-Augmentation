...
class GAN():
    ...
    def build_generator(self):                                      #responsible for producing "fake" data that mimics the real data distribution.
        ...
        model.add(LeakyReLU(alpha=0.2)) 
        model.add(Dense(self.base_n_count * 6)) #추가               #Increases the model's capacity to learn complex transformations.    
        model.add(LeakyReLU(alpha=0.2)) #추가                                                                                           
...
# combine GAN 훈련
    def train(self, epochs=1500, batch_size=13, sample_size=58):  # Observation/historical data    #Epochs (100, 200, 300, 500, 1000, 1500, 3000)
        ...
        for index in range(nb):                                                             #Index Represents the current iteration of the loop
            ...
            if (index % int(nb / 5) == 0):                                                                  
                self.fake_data_list.append(self.gen_fake_data(350)) #N=1000 train 


    def gen_fake_data(self, N=350): #N=1000                                                                  
...
def real_vs_fake(real_data, fake_data_list, x_col, y_col):                                              #Visually compares real data and fake data generated by the GAN.
    ...
    plt.savefig(r'C:\Users\Christian\Downloads\Predicting_TOC\GAN-TOC\RESULTS\1500-EPOCHS\FINALLY-2-도천-TOC_GAN_data_{}-1500-Epochs(300-SYN).png'.format(x_col),dpi=300)
...
if __name__ == '__main__':
     cc_scaled_data, col_max, cc_data = process_csv(r'C:\Users\Christian\Downloads\Predicting_TOC\GAN-TOC\INPUT-DATA\2-도천_DATA_TRAIN_MONTHLY.csv')
     gan = GAN(cc_scaled_data, col_max)
     gan.train(epochs=1500, batch_size=13, sample_size=58)  # Observation/historical data    #Epochs (100, 200, 300, 500, 1000, 1500, 3000)
     export_fakes(gan, r'C:\Users\Christian\Downloads\Predicting_TOC\GAN-TOC\RESULTS\1500-EPOCHS\FINALLY-2-도천-TOC_GAN_data-1500-Epochs(300-SYN).csv', 350)  # Number of Synthetic Data to be generated (1:3, 1:5, 1:10)
     show_gen_loss(gan)
     cc_sample_data = cc_data.sample(58)  # Observation/historical data
